...
from io_anim_bvh.import_bvh import read_bvh, bvh_node_dict2armature
...
def apply_bvh(skeleton: bpy.types.Object, bvh_bytes):
    ...
    elif first_bone and first_bone.rotation_mode in {'XYZ', 'XZY', 'YXZ', 'YZX', 'ZXY', 'ZYX'}:
    ...
    for bvh_node in bvh_nodes_list:
        ...
        if bvh_node.has_loc:
            data_path = 'pose.bones["%s"].location' % escape_identifier(
                bvh_node.name)
            location = [
                (bone_rest_matrix_inv @ Matrix.Translation(
                    Vector(
                        bvh_node.anim_data[frame_i + skip_frame][:3]) - bvh_node.rest_head_local
                )).to_translation()
        ...
        if bvh_node.has_rot:
            ...
            for frame_i in range(num_frame):
                ...
                bone_rotation_matrix = bone_rest_matrix_inv @ Euler(
                    bvh_rot, bvh_node.rot_order_str[::-1]).to_matrix().to_4x4() @ bone_rest_matrix
                if rotate_mode == 'QUATERNION':
